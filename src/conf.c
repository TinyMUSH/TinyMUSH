...
int modify_xfuncs(char *fn_name, int (*fn_ptr) (dbref), EXTFUNCS ** xfuncs, int negate)
{
	...
	tp = (NAMEDFUNC **) xrealloc(all_named_funcs, (num_named_funcs + 1) * sizeof(NAMEDFUNC *), "all_named_funcs");
	...
	*xfuncs = xfp;
     ...
     */
    tp = (NAMEDFUNC **) xrealloc(xfp->ext_funcs, (xfp->num_funcs + 1) * sizeof(NAMEDFUNC *), "nfuncs");
    tp[xfp->num_funcs] = np;
    xfp->ext_funcs = tp;
    xfp->num_funcs++;
    return 1;
