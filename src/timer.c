	...
	for (crp = cron_head; crp != NULL; crp = crp->next) {
		if ( bit_test(crp->minute, minute) && bit_test(crp->hour, hour)
			...
			if (*cmd && Good_obj(crp->obj)) {
				wait_que(crp->obj, crp->obj, 0, NOTHING, 0, cmd, (char **) NULL,
						0, NULL);
			}
...
static char *parse_cronlist(dbref player, bitstr_t *bits, int low, int high,
		char *bufp) {
...
void do_crontab(dbref player, dbref cause, int key, char *objstr) {
	...
	for (crp = cron_head; crp != NULL; crp = crp->next) {
		if (((thing == NOTHING) && (( Owner ( crp->obj )== player ) || See_Queue ( player ) ) ) ||
		( crp->obj == thing ) ) {
			count++;
	...
	notify_check(player, player, MSG_PUP_ALWAYS | MSG_ME_ALL | MSG_F_DOWN,
			"Matched %d cron %s.", count, (count == 1) ? "entry" : "entries");
}
