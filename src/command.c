	...
	if (cmdp->callseq & CS_CMDARG) {
		...
		for (add = (ADDENT *) cmdp->info.added; add != NULL; add = add->next) {
		    ...
		    *s++ = '\0';
		    ...
		    if (((!(aflags & AF_REGEXP) && wild(buff + 1, new, aargs, NUM_ENV_VARS)) || ((aflags & AF_REGEXP) && regexp_match(buff + 1, new, ((aflags & AF_CASE) ? 0 : PCRE_CASELESS), aargs, NUM_ENV_VARS)))
			&& (!mudconf.addcmd_obey_uselocks || could_doit(player, add->thing, A_LUSE))) {
			process_cmdline(((!(cmdp->callseq & CS_ACTOR)
					  || God(player)) ? add->thing : player), player, s, aargs, NUM_ENV_VARS, NULL);
...
void list_memory(dbref player)
{
	...
	each += pools[i].max_alloc * (pools[i].pool_size + sizeof(POOLHDR) + sizeof(POOLFTR));
    }
