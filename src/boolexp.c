...
static dbref lock_originator = NOTHING; /* XXX grotesque hack */
	...
	case BOOLEXP_INDIR:
		...
		if (mudstate.lock_nest_lev >= mudconf.lock_nest_lim) {
			...
			if ((mudconf.log_info & LOGOPT_LOC) && Has_location(player)) {
				...
				log_write(LOG_BUGS, "BUG", "LOCK",
						"%s in %s: Lock exceeded recursion limit.", pname,
						lname);
				free_lbuf(lname);
			} else {
				log_write(LOG_BUGS, "BUG", "LOCK",
						"%s: Lock exceeded recursion limit.", pname);
			}
		...
		if ((b->sub1->type != BOOLEXP_CONST) || (b->sub1->thing < 0)) {
			if ((mudconf.log_info & LOGOPT_LOC) && Has_location(player)) {
				lname = log_getname(Location(player));
				log_write(LOG_BUGS, "BUG", "LOCK",
						"%s in %s: Lock had bad indirection (%c, type %d)",
						pname, lname, INDIR_TOKEN, b->sub1->type);
				free_lbuf(lname);
			} else {
				log_write(LOG_BUGS, "BUG", "LOCK",
						"%s in %s: Lock had bad indirection (%c, type %d)",
						pname, INDIR_TOKEN, b->sub1->type);
			}
	...
	case BOOLEXP_EVAL:
		...
		if (checkit) {
			...
			exec(buff2, &bp, source,
					((lock_originator == NOTHING) ? player : lock_originator),
					player, EV_FCHECK | EV_EVAL | EV_TOP, &str, (char **) NULL,
					0);
...
static BOOLEXP *parse_boolexp_L(void) {
	...
	default:
		...
		while (*parsebuf && (*parsebuf != AND_TOKEN) && (*parsebuf != OR_TOKEN) && (*parsebuf != ')')) {
			*p++ = *parsebuf++;
		}
		...
		*p-- = '\0';
		...
		if (!mudstate.standalone) {
			...
			if (b->thing == AMBIGUOUS) {
				notify_check(parse_player, parse_player,
				MSG_PUP_ALWAYS | MSG_ME_ALL | MSG_F_DOWN, "I don't know which %s you mean!", buf);
				free_lbuf(buf);
				free_bool(b);
				return TRUE_BOOLEXP;
			}
