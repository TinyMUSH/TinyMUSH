...
static void
update_newobjs(player, obj_num, obj_type)
dbref player;
...
char *name;
{
    ...
    {
        notify(player,
               "The database building limit has been reached.");
        return NOTHING;
    }
...
void
do_freelist(player, cause, key, str)
dbref player, cause;
...
char *str;
{
    ...
    if (IS_CLEAN(thing))
    {
        ...
        {
            notify(player,
                   "That object is already at the head of the freelist.");
            return;
        }
        /*
        ...
        DO_WHOLE_DB(i)
        {
            if (Link(i) == thing)
            {
                ...
                {
                    notify(player,
                           "Unable to relink freelist at this time.");
                    return;
                }
            }
        }
