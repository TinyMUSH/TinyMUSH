...
int dir_index;
{
    off_t bucket_adr;       /* The address of the correct hash
                     * bucket.  */
...
int next_insert;
{
    ...
    int new_bits;           /* The number of bits for the new
                     * buckets. */
    int cache_0;            /* Location in the cache for the
                     * buckets. */
    int cache_1;
    ...
    int elem_loc;           /* Location in new bucket to put
                     * element. */
    bucket_element *old_el;     /* Pointer into the old bucket. */
    int select;         /* Used to index bucket during
                     * movement. */
    ...
    while ( dbf->bucket->count == dbf->header->bucket_elems ) {
        ...
        if ( bucket[0]->av_count == BUCKET_AVAIL ) {
            /*
             * The avail is full, move the first one to
             * bucket[1].
             */
        ...
        /*
         * Update the directory.  We have new file addresses for
         * both buckets.
         */
