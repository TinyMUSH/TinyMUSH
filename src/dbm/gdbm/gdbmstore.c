	...
	int flags;
{
	...
	off_t file_adr;			/* The address of new space in the
					 * file.  */
	off_t file_pos;			/* The position after a lseek. */
	int num_bytes;			/* Used for error detection. */
	off_t free_adr;			/* For keeping track of a freed
					 * section. */
	int free_size;
	...
	/*
	 * Check for illegal data values.  A NULL dptr field is illegal
	 * because NULL dptr returned by a lookup procedure indicates an
	 * error.
	 */
	...
	if (elem_loc == -1) {
		...
		/*
		 * We now have another element in the bucket.  Add the new
		 * information.
		 */
		dbf->bucket->count += 1;
