...
datum *return_val;
{
    ...
    while ( !found ) {
        ...
        if ( elem_loc == dbf->header->bucket_elems ) {
            /*
             * We have finished the current bucket, get the next
             * bucket.
             */
            elem_loc = 0;

            /*
             * Find the next bucket.  It is possible several
             * entries in the bucket directory point to the same
             * bucket.
             */
...
datum key;
{
    ...
    char *find_data;        /* Data pointer returned by
                     * _gdbm_findkey. */
