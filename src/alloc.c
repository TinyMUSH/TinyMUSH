...
const char *poolnames[] = { "Sbufs", "Mbufs", "Gbufs", "Lbufs", "Hbufs",
		"Bools", "Descs", "Qentries", "Pcaches" };
...
static void pool_err(const char *logsys, int logflag, int poolnum,
	...
	if (!mudstate.logging) {
		log_write(logflag, logsys, "ALLOC", "%s[%d] (tag %s) %s at %lx. (%s)",
				action, pools[poolnum].pool_size, tag, reason, (long ) ph,
				mudstate.debug_cmd);
	} else if (logflag != LOG_ALLOCATE) {
		log_write(logflag | LOG_FORCE, logsys, "ALLOC",
				"%s[%d] (tag %s) %s at %lx", action, pools[poolnum].pool_size,
				tag, reason, (long ) ph);
	}
}
...
char *pool_alloc(int poolnum, const char *tag) {
	...
	do {
		if (pools[poolnum].free_head == NULL) {
			h = (char *) malloc(
					pools[poolnum].pool_size + sizeof(POOLHDR)
							+ sizeof(POOLFTR));
	...
	for (ph = pools[poolnum].chain_head; ph != NULL; ph = ph->next) {
		if (ph->magicnum != POOL_MAGICNUM) {
			...
			notify_check(player, player,
					...
					"%d free %s (before corruption)", numfree, text);
			return;
		}
...
void *xcalloc(size_t elems, size_t esize, const char *name) {
	...
	if (mudconf.malloc_logger) {
		log_write(LOG_MALLOC, "MEM", "TRACE", "xcalloc: %s/%d", name,
				elems * esize);
		malloc_count++;
...
void *xrealloc(void *ptr, size_t amount, const char *name) {
	...
	if (mudconf.malloc_logger) {
		...
		if (ptr) {
			...
			memcpy(malloc_ptr + sizeof(int), ptr,
					*(int *) ((void *) ptr - sizeof(int)));
...
void *xstrdup(const char *str, const char *name) {
	...
	if (mudconf.malloc_logger) {
		if (str || *str) {
			malloc_str = (char *) str;
			log_write(LOG_MALLOC, "MEM", "TRACE", "xstrdup: %s/%d", name,
					strlen(malloc_str) + 1);
			malloc_count++;
...
void xfree(void *ptr, const char *name) {
	if (mudconf.malloc_logger) {
		log_write(LOG_MALLOC, "MEM", "TRACE", "Free: %s/%d", name,
				ptr ? *(int * ) ((void * ) ptr - sizeof(int)) : 0);
...
static void trace_free(const char *name, void *ptr) {
	...
	for (tptr = mudstate.raw_allocs; tptr != NULL; tptr = tptr->next) {
		if (tptr->bptr == ptr) {
			if (strcmp(tptr->buf_tag, name)) {
				log_write(LOG_BUGS, "MEM", "TRACE",
						"Free mismatch, tag %s allocated as %s", name,
						tptr->buf_tag);
			}
	...
	log_write(LOG_BUGS, "MEM", "TRACE", "Attempt to free unknown with tag %s",
			name);
}
