        ...
        if (Transparent(loc))
        {
            DOLIST(thing, Exits(parent))
            {
                if (Can_See_Exit(player, thing, isdark))
                {
                    ...
                    {
                        notify(player,
                               tprintf("%s leads home.",
                                       buff));
                    }
                    else if (Good_obj(Location(thing)))
                    {
                        notify(player,
                               tprintf("%s leads to %s.",
                                       buff,
                                       Name(Location
                                            (thing))));
                    }
...
char *name;
{
    ...
    if (control)
    {
        ...
        buf2[strlen(buf2) - 1] = '\0';
        notify(player,
               tprintf("Accessed: %s    Modified: %s", timebuf, buf2));
    ...
    if (!control)
    {
        ...
        {
            notify(player,
                   tprintf("Owned by %s", Name(Owner(thing))));
        }
    }
}
...
char *name, *qual;
{
    ...
    else
    {
        ...
        case TYPE_EXIT:
            strcpy(thingname, Name(thing));
            notify(player,
                   tprintf("@open %s", translate_string(thingname,
                           1)));
            got = thingname;
