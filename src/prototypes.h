...
/* bsd.c */
extern int get_slave_result(void);
...
extern void helper_list_cmdaccess(dbref player, CMDENT *ctab, char *buff);
...
extern lt_dlhandle lt_dlopen_format(const char *filename, ...);
extern void *lt_dlsym_format(lt_dlhandle place, const char *symbol, ...);
...
extern void makekey(dbref thing, int atr, Aname *abuff);
...
extern long random_range(long low, long high);
...
extern void helper_list_funcaccess(dbref player, FUN *fp, char *buff);
...
extern long long cvtfun(int flag, char *str);
...
extern void fun_mudname(char *buff, char **bufc, dbref player, dbref caller, dbref cause, char *fargs[], int nfargs, char *cargs[], int ncargs);
...
extern int check_command(dbref player, char *name, char *buff, char **bufc, char *cargs[], int ncargs);
...
extern void list_qpids(dbref player, dbref player_targ, dbref obj_targ, BQUE *queue, char *buff, char **bufc, char *bb_p);
...
extern int atr_has_flag(dbref player, dbref thing, ATTR *attr, int aowner, int aflags, char *flagname);
...
extern int mem_usage_attr(dbref player, char *str);
...
extern void display_nametab(dbref player, NAMETAB *ntab, char *prefix, int list_if_none);
extern void interp_nametab(dbref player, NAMETAB *ntab, int flagword, char *prefix, char *true_text, char *false_text);
extern void listset_nametab(dbref player, NAMETAB *ntab, int flagword, char *prefix, int list_if_none);
...
extern void failconn(const char *logcode, const char *logtype, const char *logreason, DESC *d, int disconnect_reason, dbref player, int filecache, char *motd_msg, char *command, char *user, char *password, char *cmdsave);
...
extern void list_sites(dbref player, SITE *site_list, const char *header_txt, int stat_type);
...
/* udb_obj.c */
extern Obj *unroll_obj(char *data);
extern char *rollup_obj(Obj *o);
extern int obj_siz(Obj *o);
extern void objfree(Obj *o);
extern char *obj_get_attrib(int anam, Obj *obj);
extern void obj_set_attrib(int anam, Obj *obj, char *value);
extern void obj_del_attrib(int anam, Obj *obj);
extern Obj *get_free_objpipe(unsigned int obj);
...
extern void cache_repl(Cache *cp, void *new, int len, unsigned int type, unsigned int flags);
...
extern DBData cache_get(DBData key, unsigned int type);
extern int cache_put(DBData key, DBData data, unsigned int type);
extern Cache *get_free_entry(int atrsize);
extern int cache_write(Cache *cp);
extern int cache_sync(void);
extern void cache_del(DBData key, unsigned int type);
...
extern DBData db_get(DBData gamekey, unsigned int type);
extern int db_put(DBData gamekey, DBData gamedata, unsigned int type);
extern int db_del(DBData gamekey, unsigned int type);
