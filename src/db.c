...
void fwdlist_set(dbref thing, FWDLIST * ifp)
{
    ...
    fp->data = (int *) xcalloc(ifp->count, sizeof(int), "fwdlist_set.data");
...
int fwdlist_load(FWDLIST * fp, dbref player, char *atext)
{
    ...
    int *tmp_array;
    tmp_array = (int *) xcalloc((LBUF_SIZE / 2), sizeof(int), "fwdlist_load.tmp");
    count = 0;
    errors = 0;
...
void propdir_set(dbref thing, PROPDIR * ifp)
{
    ...
    fp->data = (int *) xcalloc(ifp->count, sizeof(int), "propdir_set.data");
...
int propdir_load(PROPDIR * fp, dbref player, char *atext)
{
    ...
    int i, count, errors, fail;
    int *tmp_array;
    tmp_array = (int *) xcalloc((LBUF_SIZE / 2), sizeof(int), "propdir_load.tmp");
    count = 0;
    errors = 0;
...
void anum_extend(int newtop)
{
    ...
    if (anum_table == NULL) {
	anum_table = (ATTR **) xcalloc(newtop + 1, sizeof(ATTR *), "anum_extend");
    } else {
	anum_table2 = (ATTR **) xcalloc(newtop + 1, sizeof(ATTR *), "anum_extend");
...
void db_grow(dbref newtop)
{
    ...
    newnames = (NAME *) xcalloc(newsize + SIZE_HACK, sizeof(NAME), "db_grow.names");
    ...
    newnames = NULL;
    newpurenames = (NAME *) xcalloc(newsize + SIZE_HACK, sizeof(NAME), "db_grow.purenames");
