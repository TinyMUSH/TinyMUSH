...
/**
 * @brief Execute a validated command: parse switches, interpret arguments, enforce perms, and run hooks.
 *
 * Workflow highlights:
 * - Guardrails: bounds check the arg list, validate object type, and honor global build/queue gates.
 * - Permissions: run `check_cmd_access()` and switch-level access checks before dispatch.
 * - Switches: parse "/"-separated switches, enforce exclusivity, and accumulate flags into `key`.
 * - Arguments: select interpretation mode (CS_INTERP, CS_NOINTERP, /noeval), evaluate as needed,
 *   support CS_UNPARSE/CS_CMDARG/CS_ARGV, and preserve raw text when required.
 * - Dispatch: call the handler according to the call sequence, including softcode CS_ADDED handlers.
 * - Hooks: invoke registered pre/post hooks when present and permitted.
 * - Softcode commands: reconstruct command text with switches, match attributes, obey stop rules,
 *   and restore global registers around softcode execution.
 *
 * @param cmdp        Command entry containing the handler, switches, and metadata
 * @param switchp     Slash-separated switch string (e.g., "quiet/force"), or NULL if none
 * @param player      DBref of the enactor executing the command
 * @param cause       DBref of the object that caused this command execution
 * @param interactive True when invoked from direct player input; false when queued/triggered
 * @param arg         Argument string to be parsed (modified in place)
 * @param unp_command Original unparsed command string (used for logging/CS_UNPARSE)
 * @param cargs       Original command arguments array (%0-%9)
 * @param ncargs      Count of entries in `cargs`
 *
 * @return void
 *
 * @note `arg` and `switchp` are mutated during parsing; pass copies if the originals are needed.
 * @note CS_ADDED commands may execute multiple attribute matches depending on configuration.
 *
 * @see check_cmd_access() for permission validation
...
/**
 * @brief Top-level command entrypoint: normalize input, resolve the target command, and dispatch.
 *
 * Responsibilities:
 * - Validate the invoker (not GOING/Halted unless interactive player) and enforce invocation limits.
 * - Log player input, including suspect logging and optional GOD monitor output.
 * - Normalize whitespace (and space-compress if enabled) while keeping a preserved copy for logging.
 * - Allow modules to intercept or consume commands via `process_command` hooks.
 * - Fast-path single-character lead-ins, handle `home`, and respect movement hooks on exits/goto.
 * - Resolve builtins and aliases from the command hash; evaluate aliases before dispatch.
 * - Process enter/leave aliases and $-command matches across location, inventory, zones, and master room.
 * - Invoke module `process_no_match` hooks before emitting the default HUH message.
 * - Delegate to `process_cmdent()` with parsed switches/arguments when a builtin command is found.
 *
 * @param player      DBref of the enactor executing the command
 * @param cause       DBref of the object that caused this execution (often the same as player)
 * @param interactive Non-zero when invoked from direct player input; zero when queued/triggered
 * @param command     Raw command string (normalized in place)
 * @param args        Environment argument array (%0-%9)
 * @param nargs       Number of entries in `args`
 *
 * @return Pointer to a preserved copy of the normalized command string (for logging/inspection)
 *
 * @note Resets recursion/notification/lock counters and baselines the CPU timer per invocation.
 * @note Movement hooks tied to `internalgoto` are triggered elsewhere via `call_move_hook()`.
 *
 * @see process_cmdent() for switch resolution, argument interpretation, and hook invocation
 * @see call_move_hook() for movement hook processing on room transitions
 */
 ...
 * - Call `process_command__moved()` for each segment.
 *
 * Execution notes:
 ...
 * @note `process_command__moved()` handles permissions and hooks for each segment.
 *
 * @see process_command__moved()
