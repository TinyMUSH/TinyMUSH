...
/**
 * @brief Core global variables for socket management
 *
 */
...
void shovechars(int port)
{
	...
	while (mushstate.shutdown_flag == 0)
	{
		...
		if (msgrcv(msgq_Id_local, &msgq_Dns, sizeof(msgq_Dns.payload), MSGQ_DEST_REPLY - MSGQ_DEST_DNSRESOLVER, IPC_NOWAIT) > 0)
		{
	...
	msgsnd(msgq_Id_local, &msgq_Dns, sizeof(msgq_Dns.payload), 0);
