...
/* boolexp.c */
extern BOOLEXP *boolexp_alloc(void);
extern void boolexp_free(BOOLEXP *b);
extern bool boolexp_eval(dbref player, dbref thing, dbref from, BOOLEXP *b);
extern bool boolexp_eval_atr(dbref player, dbref thing, dbref from, char *key);
extern const char *skip_whitespace(const char *pBuf);
extern BOOLEXP *boolexp_parse(dbref player, const char *buf, bool internal);

/* bsd.c */
extern void bsd_main_event_loop(int port);
extern char *bsd_conn_reason_string(int reason);
extern char *bsd_conn_message_string(int reason);
extern void bsd_conn_shutdown(DESC *d, int reason);
extern DESC *bsd_conn_new(int sock);
extern DESC *bsd_sock_initialize(int s, struct sockaddr_in *a);
extern int bsd_io_output_process(DESC *d);
extern int bsd_io_input_process(DESC *d);
extern void bsd_io_chartable_init(void);
extern void bsd_conn_all_close(int emergency, char *message);
extern void bsd_status_report(void);
extern void bsd_signal_enable(void);
extern void bsd_signal_disable(void);
extern void bsd_signal_handler(int sig);
extern void bsd_signal_panic_check(int sig);
extern void bsd_signal_log(const char *signame);
...
extern bool check_mod_access(dbref player, EXTFUNCS *xperms);
extern bool check_userdef_access(dbref player, HOOKENT *hookp, char *cargs[], int ncargs);
extern void process_hook(HOOKENT *hp, int save_globs, dbref player, dbref cause, char *cargs[], int ncargs);
extern void call_move_hook(dbref player, dbref cause, bool state);
extern bool check_cmd_access(dbref player, CMDENT *cmdp, char *cargs[], int ncargs);
extern void process_cmdent(CMDENT *cmdp, char *switchp, dbref player, dbref cause, bool interactive, char *arg, char *unp_command, char *cargs[], int ncargs);
...
extern CF_Result add_helpfile(dbref player, char *confcmd, char *str, bool is_raw);
...
extern bool can_destroy_exit(dbref player, dbref exit);
extern bool destroyable(dbref victim);
extern bool can_destroy_player(dbref player, dbref victim);
...
extern char *getstring(FILE *f, bool new_strings);
extern dbref getref(FILE *f);
extern long getlong(FILE *f);
extern int init_database(char *dbfile);
extern bool check_zone(dbref player, dbref thing);
extern bool check_zone_for_player(dbref player, dbref thing);
...
extern bool Commer(dbref thing);
extern void al_extend(char **buffer, int *bufsiz, int len, bool copy);
...
extern bool atr_get_info(dbref thing, int atr, dbref *owner, int *flags);
...
extern FILE *db_module_flatfile(char *filename, bool wrflag);
...
extern bool fn_range_check(const char *fname, int nfargs, int minargs, int maxargs, char *result, char **bufc);
...
extern bool escaped_chars(unsigned char ch);
...
extern bool cvtfun(int flag, char *str);
...
extern bool do_convtime(char *str, struct tm *ttm);
...
extern bool check_command(dbref player, char *name, char *buff, char **bufc, char *cargs[], int ncargs);
...
extern bool atr_has_flag(dbref player, dbref thing, ATTR *attr, int aowner, int aflags, char *flagname);
...
extern void display_nametab(dbref player, NAMETAB *ntab, bool list_if_none, const char *format, ...);
extern void interp_nametab(dbref player, NAMETAB *ntab, int flagword, char *prefix, char *state, char *true_text, char *false_text, bool show_sep);
extern void listset_nametab(dbref player, NAMETAB *ntab, int flagword, bool list_if_none, char *format, ...);
...
extern void list_sites(dbref player, SITE *site_list, const char *header_txt, int stat_type, bool header, bool footer);
...
extern bool can_set_home(dbref player, dbref thing, dbref home);
...
extern bool is_integer(char *str);
